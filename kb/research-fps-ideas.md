...
### F5. I-cache packing by linker placement — ⭐ BUILT 2026-08-09, UNMEASURED
...
✅ **SHIPPED AND ON BY DEFAULT.** `dc/section-order.txt` is generated by
`tools/dcopt/icache_map.py --emit-order` and passed to the link as
`-Wl,--section-ordering-file`. Kill switch **`DC_SECTION_ORDER=0`**, which
returns an argv-identical link line. Full knob + format documentation:
`BUILDING-DC.md` § `DC_SECTION_ORDER`. **It has never been run.**

⚠️ **THE OLD PRESSURE FIGURES ON THIS PAGE WERE WRONG AND ARE CORRECTED.** They
said "11.9x the icache" and "the 12-symbol innermost draw loop is 1.4x it". Both
were computed over a hot set that **did not contain emu64 at all**:
`icache_map.py`'s default patterns were `^_dl_G_`, `^_emu64`, `^_cu_trin` — but
emu64 is C++, so every opcode handler is a mangled name (`__ZN5emu649dl_G_TRINEv`,
not `_dl_G_TRIN`) and those three patterns matched **nothing**. The interpreter
that G1 measured as most of the draw was absent from the measurement of its own
cache pressure. Corrected on `dc/build/AnimalCrossing.elf`, 2026-08-09:

| | symbols | bytes | vs the 8 KB cache |
|---|---|---|---|
| sized `.text` | 15,305 | 2,469,026 | — |
| hot set (corrected patterns) | 336 | 134,382 | **16.40x** |
| tier 1, the innermost draw loop | 33 | 21,424 | **2.62x** |

The inner loop is **2.62x the cache, not 1.4x** — so ordering cannot make it
resident, only contiguous. Contiguity is still the right move (it converts a
scattered conflict pattern into a predictable capacity one), but the ceiling on
this idea is lower than the old number implied.

- ⚠️ **`--symbol-ordering-file` is an LLD flag and we use GNU ld.** 2.45.1 has
  **`--section-ordering-file FILE`**, which does the same job — but it takes a
  linker-script `SECTIONS` **fragment**, not a list of names, and the section
  name drops the symbol's leading underscore. Both verified; see the
  `icache_map.py` docstring, which is the reference.
- ⚠️ **The ordering file must open with a bare `*(.text)`** or `_kos_startup.o`'s
  `start` stops being the first byte at `0x8c010000` and the image does not boot.
