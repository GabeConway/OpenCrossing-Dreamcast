...
|---|---|---|
| `kb/design-shelf-exclusions.md` | §0, §1, §2 | the 12 TUs that fail, `pc/CMakeLists.txt` read in full, the two `-O2` confounds, the DC exclusion/fix list |
| `kb/design-shelf-flags.md` | §3, §7, §9 | the flag set justified per flag, SH-4 GCC probe findings, the `-O2` risk verdict (§3.4's `-O2` recommendation is withdrawn — see `kb/research-size-reduction.md`) |
