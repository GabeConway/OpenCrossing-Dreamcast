...
| `DC_ARAM_AUDIO_DROP` | **derived `0` when `DC_AUDIO=1`** (else `1`) | ⭐ **NO LONGER A MANUAL RECIPE — `dc/Makefile` derives it from `DC_AUDIO=1` with `?=` (2026-08-08).** It had been documented here and in `dc/Makefile` as something you must remember to pass, and wired nowhere, which is how a whole session shipped silent builds. At `1` with sound on, all 8,300,384 B of `audiorom.img` is discarded at `dc_aram.c:313-320` *before* `dc_dvd_provenance()`, the audio half of ARAM gets zero extents, and every jaudio sample fetch is `memset` to zero: a live pipe carrying silence, with `[NEOS_OUT] peak=0` as the only tell. **PROVEN GOOD 2026-08-06**: `mapped` 4,982,400 → **13,282,784** (exactly +8,300,384), `ext=3/32`, `LOST=0`, `drop=0`, `zero=0`, `pin_peak=0`, `peak` 0 → 3851, human-verified on real hardware. Costs no pool bytes. Set it to `1` explicitly only to bisect the ordering hazard at `dc_aram.c:295-312` |
| `DC_AUDIO` | `0` | `1` → open the AICA output device and run the jaudio synthesis pump. ⚠️ **The "~45 % of the frame rate" figure that used to live here is VOID** — it was measured at `-O0`, before the port-queue drain, and with the peak-cost levers off. Measured 2026-08-08 with all three fixed: town `draw` 49.9 ms and `fps_p50` 23.2 **with sound on**, against 45.6 ms for the old silent build. Setting `DC_AUDIO=1` now also derives `DC_ARAM_AUDIO_DROP=0`, `DC_AUDIO_MIXRATE=24000` and `DC_AUDIO_SUBDELAY=0` |
...
| `DC_AUDIO_MIXRATE` | **`24000` when `DC_AUDIO=1`** | **L2.** jaudio's INTERNAL mix rate (`NA_SPEC_CONFIG[0]._00`, shipped 48000) — not the output rate. Halves `num_samples_per_update` 200 → 96 (`memory.c:974`) and with it the resampler, the envelope mixer, the reverb bus and the velocity-convolution copies. **Pitch is preserved**: the resample error term is independent of this field. ⚠️ Does **not** cut ADPCM decode, and it pushes more notes over the `pitch >= 2.0` two-part threshold (`channel.c:129`), which claws some back. Pick a value whose `rate/60/4` is a multiple of 8. `0` restores 48000 |
| `DC_AUDIO_SUBDELAY` | **`0` when `DC_AUDIO=1`** | **L5.** `NA_HALL_DELAY.sub_delay` (shipped `0x14`) — the hall reverb's **second** tap. `0` deletes ~16 KB of memcpy and ~1,664 MACs per DAC frame and costs the echo tail only; the primary reverb is untouched and `leak_rtl`/`leak_ltl` are already 0 in the shipped table. Every consumer has an else branch, which is what makes it cheap to switch off. `-1` restores `0x14`. ⚠️ **NOT the same lever as `NA_SPEC_CONFIG[0]._09`** — zeroing that one NULL-derefs at `driver.c:786` |
| `DC_AUDIO_VOICES` | unset (= shipped 24) | **L1, priced and unused.** `NA_SPEC_CONFIG[0]._05`. Per-voice cost is exactly linear (`~265 us` per voice-update, measured 2026-08-08), so this caps the WORST frame and barely moves the mean; peak in the town is 14-15 concurrent voices. Degrades to priority-ordered note stealing via `__Nas_GetLowerPrio` (`channel.c:788-811`) — no new drop logic. ⚠️ It is **no longer a safety requirement**: S8e grew `pc_task_buf` to jaudio's own 2,350-command bound |
| `DC_AUDIO_NO_DRAIN` | unset | `DC_XDEFS` only. Reverts the port-queue drain in `dc_audio_pump()` — i.e. **puts the "music never plays" bug back**. Kept for bisection: with it set, `SendStart::Mesg Full Queue` reappears and BGM's once-per-scene `START_SEQ` gets eaten |
| `DC_EMU64_CULL` | **`1`** | **G3 — the AABB cull at `G_TRIN_INDEPEND` entry.** `<N>` is a sampling period in presented frames: `1` armed always, `N>1` armed one frame in N (the matched-frame A/B), `0` compiles `dc/src/dc_emu64_cull.cpp` to nothing. Measured **−19.9 ms of a 69.8 ms town frame**, `fps_p50` 19.5 → 23.2, and the late cull's `vcull` 9,915 → 1,002. Prints `[EMU64C] trin= cull= vis= punt= refs= \| cum reinst=`; **`reinst` must be 0** — non-zero means an installer ordering bug against G1/G2 |
| `DC_EMU64_CULL_VERIFY` | `0` | G3's correctness gate. **Never culls**: runs the entry test AND the original handler, then checks that `dc_gx.c`'s late cull agreed. Adds `falsecull=` / `gfxp_bad=` / `nocmp=` to the `[EMU64C]` line; **`falsecull` and `gfxp_bad` must both be 0** (`nocmp` is frameskipped ticks, where the oracle cannot answer — not a fault). A correctness run, never a perf run: it is slower than a clean build by construction. `#error`s against `-DDC_NO_BATCH_CULL`, which would make its verdict meaningless |
...
| `DC_PVR_VTXSPLIT` | unset | `<N>` → **G5, the split of `[PHASE] xform=`** into seven stages (`memo`/`xf`/`lit`/`tex`/`shade`/`post`/`emit`), sampling one primitive in N. Prints `[VTXSPLIT]` per presented frame with `sum=` against `xform=` so the residual is visible. **Ran 2026-08-08 at N=16: the frame is memory-bound and all the FP is 0.81 ms of 30.** ⚠️ TMU2's tick is 80 ns and a stage is 100-600 ns — only the window mean is meaningful, and a `0.00` bucket means "below the noise", not "free" |
| `DC_PMCR` | unset | `1` → **P1, the SH7750 performance counters** (`dc/src/dc_pmcr.c`). Brackets the frame with **PRFC1** (KOS owns PRFC0) and rotates it through 8 events — elapsed cycles, icache/dcache pipeline-freeze cycles, icache/operand miss counts, fill cycles, instructions issued — printing `[PMCR]` per PRESENTED frame with the same `draw`/`skip`/`vi` split as `[PHASE]`, plus `audio` and `xform`. ⚠️ **Flycast implements no PMCR and returns 0 for every event**; this is a burn-only measurement, and the instrument says so itself when it sees a zero window |
| `DC_PMCR_DWELL` | `1` | report windows (30 presented frames each) per event before rotating. A full 8-event table is ~12 s at 20 FPS |
| `DC_PMCR_HUD` | unset | `1` → draw the rotating table onto the TV, straight into the scanned-out framebuffer (address read from `FB_R_SOF1` every frame). **This is what makes a burn readable.** Costs a screenful of uncached VRAM writes per frame, measured and reported as `hud=`, which is *inside* `vi=` |
| `DC_PMCR_LOG` | HUD ? `0` : `1` | force the `[PMCR]` console line on (`1`) or off (`0`). With the HUD compiled in it defaults OFF, because on hardware a console line is charged to the frame it describes |
| `DC_CONSOLE_MUTE` | unset | `1` → `dbgio_disable()` at the top of `main()`: **no console output at all**. For measuring burns. KOS busy-waits on the SCIF FIFO with or without a cable, and a perf build puts ~10 lines into every 30-frame window — the log would be measuring itself. ⚠️ **Silences crash dumps too**; leave it off for a triage burn |
...
# ARAM_AUDIO_DROP=0 and the two peak-cost levers are DERIVED now -- nothing else to pass
DC_AUDIO_SCENES=3 bash dc/build-dc.sh
```
...
~~**⚠️ Also pass `-DDC_ARAM_AUDIO_DROP=0`.**~~ ✅ **DERIVED SINCE 2026-08-08** —
`dc/Makefile` sets it from `DC_AUDIO=1`. It is recorded here because it was a
manual step for two sessions and silently produced silent builds at full CPU
cost with every audio counter looking healthy.
