...
| `kb/audio-aica-runtime.md` | **stage B's RUNTIME — built, linked, RUNS; never heard, no FPS number.** Read before touching `dc/src/dc_aica*.c`. Default OFF and byte-identical there. ⭐ §4 is the finding that matters: FIR/comb/Haas **never fire at all** and the reverb send alone blocks ~58 % of eligible voices, which re-prices the AICA DSP send from a footnote to the main item. §3 is the `--wrap` spelling trap; 🔴 §7 is a "graphics fault" chased for most of a session that was an OMITTED `DC_STUB_KEEP` — ~53 KB of assets instead of 2.9 MB, with every counter green. **Use `kb/RESUME.md` §2's build line verbatim** |
