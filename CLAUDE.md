...
| `kb/research-sh4zam-gap.md` | **what we are missing from sh4zam**, audited against our pipeline + a teardown of 20 showcase projects, 2026-08-08. ⚠️ §0 corrects the obvious reading: **we already emit FTRV/FIPR/FSRRA — through KOS** — and sh4zam ships zero instructions today. ⚠️ **§0a REOPENS `kb/RESUME.md`'s "the per-lit-vertex block is ALREADY OPTIMAL"** — back-to-back FIPRs stall 4-5 cycles each and 3+ dots against a constant vector is an FTRV. Ranked gaps **G-A** (build bug: the `source/sh4/*` files are never compiled, so `shz_sq_memcpy32`/`shz_fft` cannot link) → **G-B** (transform each unique vertex once, before index expansion) → **G-C** (no PVR Direct Rendering, a vertex is touched 4-5×). §2 is what is dead; §4e is the trap list other people paid for |
